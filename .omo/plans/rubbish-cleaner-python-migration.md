...
- [ ] 1. requirements.txt + .gitignore + feature branch
...
- [ ] 2. scripts/lib/platform.py — cross-platform detection layer
...
- [ ] 3. scripts/lib/core.py — safety primitives (delete, quarantine, CSV, junction walk, parallel)
...
- [ ] 4. scripts/scanner.py — 15-category read-only classifier + checkpoint/resume
...
- [ ] 5. scripts/cleaner.py — approval-gated safe cleanup
...
- [ ] 6. scripts/report.py — 8-section summary verification report
...
- [ ] 7. scripts/install.py — copy skill to 3 agent dirs
...
- [ ] 8. scripts/schedule.py — Task Scheduler / cron / launchd
...
- [ ] 9. tests/test_runner.py — dual-mode test runner (compileall + pytest-or-fallback)
...
- [ ] 10. .github/workflows/test.yml — Python CI matrix
...
- [ ] 11. SKILL.md + SKILL_zh.md + README.md + README_zh.md + CHANGELOG.md + CHANGELOG_zh.md — docs update
...
- [ ] 12. tests/ — 6 pytest test files mirroring PS behavior matrix
...
- [ ] 13. Delete deprecated .ps1 files + .gitignore cleanup
...
- [ ] 14. Git finalize — push feature + merge --no-ff main + push main + install re-sync
...
- [ ] F1. Plan compliance audit — verifier diffs executed repo against plan: all 14 todos `- [x]`; every .py file exists (platform, core, scanner, cleaner, report, install, schedule, test_runner, 6 test_*.py); zero .ps1 tracked; CI updated; docs updated; CHANGELOG has v2.0.0; git convention satisfied; worktree clean
- [ ] F2. Code quality review — verifier checks: all .py files compile (`compileall` 0 errors); Python 3.10+ compatible (no `match` statement, no `X | Y` union, no walrus `:=` outside tests); `-LiteralPath`/junction safety invariants ported to Python equivalents (`is_symlink` + `is_junction` skips); `safe_remove`/`quarantine` per-item try/catch with correct disposition mapping; `parallel_for_each` uses ThreadPoolExecutor; no `eval`/`exec` on user data; `requirements.txt` has psutil + pywin32 (optional); no new heavy deps beyond those two; SKILL.md < 40KB
- [ ] F3. Agent-executed end-to-end QA — verifier runs: `python tests/test_runner.py` (exit 0, all suites pass, BRANCH marker); `python scripts/install.py` (3 agent dirs with .py only); read-only smoke `python scripts/scanner.py -Drive D: -Categories root-temps,root-logs` (exit 0, outputs, delta <500MB); `python scripts/schedule.py list` (exit 0); sandbox clean-run safety verified (nothing outside tmp_path deleted); all evidence files present
- [ ] F4. Scope fidelity — verifier checks: 9 new .py files + 7 test .py + 6 updated docs + 2 updated configs; 14 .ps1 removed; the 9 deduplicated requirements from v1.1.0 are all ported (cross-platform, test-drive, multithreading, progress/resume, CI, schedule, multi-drive, CHANGELOG, bilingual/hyperlinks); no scope creep (no new features beyond the language port); PS 5.1 retired; constraints honored
