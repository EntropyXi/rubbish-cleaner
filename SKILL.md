...
## 3. 铁律（5 条，完整护栏见 `references/safety-rules.md`）
...
15 个分类的精确匹配规则与逐类 MUST-NOT 见 `references/junk-taxonomy.md`；
应用路径模板见 `references/per-app-path-map.md`。
...
完整清单见 `references/safety-rules.md` 第 10、11 节；此处仅列核心红线：
